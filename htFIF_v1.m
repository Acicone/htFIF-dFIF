function [IMF,logM] = htFIF_v1(f,tau,options,M)


%
%  function IMF = htFIF_v1(f,options)
%
...
%
%   tau        Threshold value. Default value 0.9
%
...
if nargin < 1,  help htFIF_v1; return; end
if nargin < 2, tau=0.9; end
if nargin < 3, options = Settings_IF_v1; end
if nargin < 4, M = []; end
...
while countIMFs < options.IF.NIMFs && k_pp>=options.IF.ExtPoints && toc<options.maxTime
    ...
    if options.saveInter==1
        save([nameFile '_intermediate_htFIF_v1.mat'],'IMF','f','logM','-v7.3');
    end
end %end of while
...
if options.saveEnd == 1
    save([ 'Final_' nameFile '_htFIF_v1.mat'],'IMF','logM','-v7.3');
end
