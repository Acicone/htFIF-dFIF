function [IMF,logM] = dFIF_v1(f,tau,kappa,options,M)


%
%  function IMF = dFIF_v1(f,options)
%
...
%
%   tau        Threshold value. Default value 0.98
%
%   kappa      Used in the Soft Thresholding to define the new value of the
%              thresholded eigenvalues. Default value 0.56
%
...
%
%   See also SETTINGS_IF_V1, GET_MASK_V1, MAXMINS_v3_4, PLOT_IMF_V8.
%
...
if nargin < 1,  help dFIF_v1; return; end
if nargin < 2, tau=0.98; end
if nargin < 3, kappa=0.56; end
if nargin < 4, options = Settings_IF_v1; end
if nargin < 5, M = []; end
...
while countIMFs < options.IF.NIMFs && k_pp>=options.IF.ExtPoints && toc<options.maxTime
    ...
    %%%%%%%%%%%%%%%%%% generating f_n %%%%%%%%%%%%%%%%%%
    B=(ones(1,length(ifftA))-ifftA);
    
    val=max(B(B<tau));
    N0=round(log(kappa)/log(val));
    h=ifft((B.^N0).*fftH);
    if options.verbose>0
        sprintf('dFIF with N_0 = %d',N0)
    end
    ...
    if options.saveInter==1
        save([nameFile '_intermediate_dFIF_v1.mat'],'IMF','f','logM','-v7.3');
    end
end %end of while
...
if options.saveEnd == 1
    save([ 'Final_' nameFile '_dFIF_v1.mat'],'IMF','logM','-v7.3');
end
